...
export default function signup() {
    ...
    const handleRegister = async (e) => {
        const registerUrl = "http://localhost:1337/api/users"
        e.preventDefault();
            const registerInfo = {
                username: username,
                email:email,
    ...
    <div className="flex justify-center items-center h-screen bg-slate-200">
        <div id="form" className="block bg-slate-50 p-6 rounded-xl shodow-md shadow-slate-300 w-90">
            <form method='POST' onSubmit={handleRegister}>
                <h2 className="text-blue-700 text-3xl font-semibold my-4">Register Form</h2>
                {/* <!-- full name --> */}
                ...
                <input type="emial" name="" id="email" value={email} onChange={event=> setEmail(event.target.value)}
                    className="h-8 w-full rounded-md border border-slate-300 text-sm pl-2 bg-transparent outline-blue-600 shadow-sm"/>
                {/* <!-- password --> */}
                ...
                <button type="submit" name="" id="signUp" 
                    className="bg-blue-700 w-full h-10 cursor-pointer text-white rounded-md hover:bg-blue-600 hover:outline outline-2 outline-blue-600 outline-offset-2 text-sm">Signup</button><br/>
                <p className="text-xs my-2">Already have a account? <a href="#" className="text-blue-600"><Link href={"/login"}>Login</Link></a></p>
            </form>
        </div>
        </div>
  )
